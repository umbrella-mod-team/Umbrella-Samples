...
namespace WIGUx.Modules.fzeroaxdeluxeMotionSim
{
    public class fzeroaxdeluxeMotionSimController : MonoBehaviour
    {
        ...
        public Light turbo_light1;
        public Light turbo_light2;
        private Renderer[] AttractEmissiveObjects;
        public float attractFlashDuration = 0.7f;
        public float attractFlashDelay = 0.7f;
        public float dangerFlashDuration = 0.3f;
        public float dangerFlashDelay = 0.3f;
        ...
        void StartFocusMode()
        {
            logger.Debug("Starting Focus Mode...");
            StopCurrentPatterns();
            StartDangerPattern();
            ...
            if (cockpitCollider != null && inside)
            {
                ...
                if (playerVRSetup != null)
                {
                    ...
                    logger.Debug($"{gameObject.name} F-Zero AX Motion Sim starting... O.K. Captain Falcon, You Ready?");
                    logger.Debug($"{gameObject.name} Here We Go!...");
                    isRiding = true; // Set riding state to true
                }
            }
            else
 ...
 isCenteringRotation = false;
            ...
            if (XInput.Get(XInput.Button.RIndexTrigger))
            {
                ...
                // For throttle down, negative direction: decrease currentRotationY toward minRotationY
                float distanceToLimit = currentRotationY - minRotationY;
                float rotateY = Mathf.Min(inputValue, distanceToLimit);

                if (Mathf.Abs(rotateY) > 0.0001f)
                {
                    YObject.localRotation *= Quaternion.Euler(0, -rotateY, 0);
 ...
 isCenteringRotation = false;
            ...
            if (XInput.Get(XInput.Button.LIndexTrigger))
            {
                ...
                // For throttle up, positive direction: increase currentRotationY toward maxRotationY
 ...
 isCenteringRotation = false;
        ...
        void SaveOriginalParent(GameObject obj)        // Save original parent of object in dictionary
        {
        ...
        void RestoreOriginalParent(GameObject obj, string name)        // Restore original parent of object and log appropriate message
        {
            if (obj != null && originalParents.ContainsKey(obj))
            {
        ...
        {
            foreach (var renderer in AttractEmissiveObjects)
            {
        ...
        void InitializeObjects()
        {
            ...
            if (XObject != null)
            {
                ...
                XStartPosition = XObject.transform.localPosition;
                XStartRotation = XObject.transform.localRotation;
                ...
                if (YObject != null)
                {
                    ...
                    YStartPosition = YObject.transform.localPosition;
                    YStartRotation = YObject.transform.localRotation;
                    ...
                    if (ZObject != null)
                    {
                        ...
                        ZStartPosition = ZObject.transform.localPosition;
                        ZStartRotation = ZObject.transform.localRotation;
            ...
            AttractEmissiveObjects = new Renderer[5];
            // Look For attract emissive objects under the "buttons" parent
            for (int i = 0; i < 5; i++)
            {
                AttractEmissiveObjects[i] = transform.Find("Buttons/attractEmissive" + (i + 1))?.GetComponent<Renderer>();
            }
        }
