...
namespace WIGUx.Modules.radmMotionSim
{
    public class radmMotionSimController : MonoBehaviour
    {
        ...
        [SerializeField] float minRotationZ = -12f;
        [SerializeField] float maxRotationZ = 12f;
        ...
        public Light Start_lamp;
        public Light Lights_lamp;
        public Light Wiper_lamp;
        private Transform StartObject;
        private Transform BrakelightsObject;
        private Transform WiperObject;
        ...
        Dictionary<string, int> lastLampStates = new Dictionary<string, int>
             {
                  { "Start_lamp", 0 }, { "Wiper_lamp", 0 }, { "Lights_lamp", 0 }
             };
        ...
        void Start()
        {
            ...
            if (Start_lamp) ToggleLight(Start_lamp, false);
            if (StartObject) ToggleEmissive(StartObject.gameObject, false);
            if (Wiper_lamp) ToggleLight(Wiper_lamp, false);
            if (StartObject) ToggleEmissive(StartObject.gameObject, false);
            if (Lights_lamp) ToggleLight(Lights_lamp, false);
            if (StartObject) ToggleEmissive(StartObject.gameObject, false);
            StartAttractPattern();
        }
        ...
        void StartFocusMode()
        {
            ...
            if (cockpitCollider != null && inside)
            {
                if (playerVRSetup == null)
                {
                    ...
                    isRiding = true; // Set riding state to false
                }
                if (playerVRSetup != null)
                {
                    ...
                    logger.Debug("Hanging Sonic on the Mirror...");
                    logger.Debug("Sega Rad Mobile Deluxe Motion Sim starting...");
                    logger.Debug("El queso está viejo y podrido. Dónde está el sanitario");
                    isRiding = true; // Set riding state to false
                }
            }
            else
 ...
 isCenteringRotation = false;
        ...
        private void MapButtons(ref bool inputDetected, ref bool throttleDetected)
        {
            ...
            if (XInput.IsConnected && XInput.GetDown(XInput.Button.LIndexTrigger))
            {
 ...
 isCenteringRotation = false;
            ...
            if (XInput.IsConnected && XInput.GetUp(XInput.Button.LIndexTrigger))
            {
                if (brake1_light) ToggleLight(brake1_light, false);
                if (brake2_light) ToggleLight(brake2_light, false);
                if (BrakelightsObject) ToggleEmissive(BrakelightsObject.gameObject, false);
 ...
 isCenteringRotation = false;
    ...
    {
        { "Start_lamp", 0 }, { "Lights_lamp", 0 }, { "Wiper_lamp", 0 }
    };
        ...
        void ProcessLamp0(int state)
        {
            ...
            // Update lights
            if (Start_lamp) ToggleLight(Start_lamp, state == 1);
            // Update emissive material
            if (StartObject) ToggleEmissive(StartObject.gameObject, state == 1);
        }
        // Individual function for Lights_lamp
        void ProcessLamp1(int state)
        {
            logger.Debug($"Lights_lamp updated: {state}");

            // Update lights
            if (Lights_lamp) ToggleLight(Lights_lamp, state == 1);
            // Update emissive material
            if (LightsObject) ToggleEmissive(LightsObject.gameObject, state == 1);
        }
        // Individual function for lamp0
        void ProcessLamp2(int state)
        {
            logger.Debug($"Wiper_lamp updated: {state}");

            // Update lights
            if (Wiper_lamp) ToggleLight(Wiper_lamp, state == 1);
            // Update emissive material
            if (WiperObject) ToggleEmissive(WiperObject.gameObject, state == 1);
        }
            ...
            if (XObject != null)
            {
                ...
                if (YObject != null)
                {
                    ...
                    if (ZObject != null)
                    {
                        ...
                        // Find WiperObject object under Z
                        WiperObject = ZObject.Find("Wiper");
                        if (WiperObject != null)
                        {
                            logger.Debug($"{gameObject.name} Wiper object found.");
                            // Ensure the Wiper object is initially off
                            Renderer renderer = WiperObject.GetComponent<Renderer>();
                            if (renderer != null)
                            {
                                renderer.material.DisableKeyword("_EMISSION");
                            }
                        ...
                        {
                            logger.Debug($"{gameObject.name} Wiper object not found.");
                        }
                        // Find StartObject object under Z
                        LightsObject = ZObject.Find("Lights");
                        if (LightsObject != null)
                        {
                            logger.Debug($"{gameObject.name} Lights object found.");
                            // Ensure the Lights object is initially off
                            Renderer renderer = LightsObject.GetComponent<Renderer>();
                            if (renderer != null)
                            {
                                renderer.material.DisableKeyword("_EMISSION");
                            }
                        ...
                        {
                            logger.Debug($"{gameObject.name} Lights object not found.");
                        }
                        // Find Brakelights object under Z
                        BrakelightsObject = ZObject.Find("Brakelights");
                        if (BrakelightsObject != null)
                        {
                            logger.Debug($"{gameObject.name} Brakelights object found.");
                            // Ensure the Brakelights object is initially off
                            Renderer renderer = BrakelightsObject.GetComponent<Renderer>();
                            if (renderer != null)
                            {
                                renderer.material.DisableKeyword("_EMISSION");
                            }
