...
namespace CaptureCoreCompanion
{
    public partial class Xbox360Form : Form
    {
        ...
        public Xbox360Form(string xmlFile, string emuVRPath, string defaultOutputFolder)
        {
            this.emuVRPath = emuVRPath;
            xmlFilePath = xmlFile;
            InitializeComponent();
            // Set default emulator path
            if (string.IsNullOrWhiteSpace(txtXeniaPath.Text))
                txtXeniaPath.Text = Path.GetFullPath(Path.Combine(emuVRPath, @".\Emulators\Xenia\Xenia.exe"));
            // Set XBLA game folders
            if (string.IsNullOrWhiteSpace(txtInstalledFolder.Text))
                txtInstalledFolder.Text = Path.GetFullPath(Path.Combine(emuVRPath, @".\Roms\Microsoft XBLA\"));
            // Set Iso Games folders
            if (string.IsNullOrWhiteSpace(txtXbox360Folder.Text))
                txtXbox360Folder.Text = Path.GetFullPath(Path.Combine(emuVRPath, @".\Roms\Microsoft Xbox 360\"));
            // Set God Games folders
            if (string.IsNullOrWhiteSpace(txtGODFolder.Text))
                txtGODFolder.Text = Path.GetFullPath(Path.Combine(emuVRPath, @".\Roms\Microsoft Xbox 360\"));
            // Set output folders
            if (string.IsNullOrWhiteSpace(txtOutputXBLA.Text))
                txtOutputXBLA.Text = Path.GetFullPath(Path.Combine(emuVRPath, @".\Games\Microsoft XBLA\"));
            if (string.IsNullOrWhiteSpace(txtOutputXbox360.Text))
                txtOutputXbox360.Text = Path.GetFullPath(Path.Combine(emuVRPath, @".\Games\Microsoft Xbox 360\"));
        }
        ...
        private void CreateGameFiles(string title, string path, string output, string exe)
        {
            ...
            // Respect relative paths checkbox
            string finalPath = MakePathRelativeIfInside(path, emuVRPath);

            using (var w = new StreamWriter(Path.Combine(output, $"{safe}.bat")))
            {
                w.WriteLine("cd ./Games/Arcade (Capture)");
                w.WriteLine("cd ../..");
                w.WriteLine($"\"{exe}\" --fullscreen \"{finalPath}\"");
            }
        }
