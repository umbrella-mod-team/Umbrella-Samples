            ...
            if (XObject != null)
            {
                ...
                if (YObject != null)
                {
                    ...
                    if (ZObject != null)
                    {
                        ...
                        // Find hazard object under Z
                        hazardObject = ZObject.Find("hazard");
                        if (hazardObject != null)
                        {
                            logger.Info("hazard object found.");

                            // Ensure the hazard object's emission is initially off for all materials
                            Renderer renderer = hazardObject.GetComponent<Renderer>();
                            if (renderer != null)
                            {
                            ...
                            {
                                logger.Debug("Renderer component is not found on hazard object.");
                            }
                        }
                        else
                        {
                            logger.Debug("hazard object not found under Z.");
                        }
        ...
        void ToggleBrakeEmissive(bool isActive)
        {
            if (hazardObject != null)
            {
                ...
                if (renderer != null)
                {
                    ...
                    // logger.Info($"hazardObject object emission turned {(isActive ? "on" : "off")}.");
                }
                else
                {
                    logger.Debug("Renderer component is not found on hazardObject object.");
                }
            }
            else
            {
                logger.Debug("hazardObject object is not assigned.");
            }
        }
