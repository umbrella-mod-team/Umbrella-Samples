...
namespace WIGUx.Modules.idlimitedMotionSim
{
    public class idlimitedMotionSimController : MonoBehaviour
    {
        ...
        public float DangerDuration = 0.7f;
        public float DangerDelay = 0.7f;
        ...
        void Start()
        {
                ...
                if (YObject != null)
                {
                    ...
                    if (ZObject != null)
                    {
                        ...
                        if (HazardObject != null)
                        {
                            ...
                            {
                                logger.Debug($"{gameObject.name} Renderer component is not found on Hazard object.");
                            }
                        }
                        else
                        {
                            logger.Debug($"{gameObject.name} Hazard object not found under Z.");
                        }
                        ...
                        if (ControllerX != null)
                        {
                            ...
                            if (ControllerY != null)
                            {
                                ...
                                {
                                    logger.Debug($"{gameObject.name} ControllerZ object not found under ControllerY!");
                                }
                            }
                            else
                            {
                                logger.Debug($"{gameObject.name} ControllerY object not found under ControllerX!");
                            }
                        }
                        else
                        {
                            logger.Debug($"{gameObject.name} ControllerX object not found under Z!");
                        }
                        // Gets all Light components in the target object and its children
                        Light[] lights = ZObject.GetComponentsInChildren<Light>();
                        ...
                        {
                            logger.Debug($"{gameObject.name} Cockpitcam object not found under Z!");
                        }
                    }
                    else
                    {
                        logger.Debug($"{gameObject.name} Z object not found under Y!");
                    }
                }
                else
                {
                    logger.Debug($"{gameObject.name} Y object not found under X!");
                }
            }
            else
            {
            ...
            HazzardEmissiveObjects[0] = ZObject.Find("Hazzard")?.GetComponent<Renderer>();
            HazzardEmissiveObjects[1] = ZObject.Find("HazzardL")?.GetComponent<Renderer>();
            HazzardEmissiveObjects[2] = ZObject.Find("HazzardR")?.GetComponent<Renderer>();
        ...
        // Method to toggle the BrakeEmissive object
        void ToggleBrakeEmissive(bool isActive)
        {
            ...
            {
                logger.Debug("Hazard object is not assigned.");
            }
        }
        ...
        public void StartDangerPattern()
        {
            ...
            attractCoroutine = StartCoroutine(HazzardPattern());
        }
