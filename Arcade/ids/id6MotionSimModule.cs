...
namespace WIGUx.Modules.idMotionSim
{
    public class idMotionSimController : MonoBehaviour
    {
        ...
        private Vector3 XStartPosition;
        private Quaternion XStartRotation;
        private Vector3 YStartPosition;
        private Quaternion YStartRotation;
        private Vector3 ZStartPosition;
        private Quaternion ZStartRotation;
        ...
        private Quaternion ControllerXStartRotation; // Initial controlller positions and rotations for resetting
        ...
        private Quaternion ControllerYStartRotation; // Initial controlller positions and rotations for resetting
        ...
        private Quaternion ControllerZStartRotation; // Initial controlller positions and rotations for resetting
        ...
        private Transform taillight1Object;
        private Transform taillight2Object;
        ...
        void Start()
        {
                ...
                if (YObject != null)
                {
                    ...
                    if (ZObject != null)
                    {
                        ...
                        if (taillight1Object != null)
                        {
                            ...
                            {
                                logger.Debug("Renderer component is not found on taillight1 object.");
                            }
                        }
                        else
                        {
                            logger.Debug("taillight1 object not found under Z.");
                        }

                        // Find taillight2 object under Z
                        taillight2Object = ZObject.Find("taillight2");
                        if (taillight2Object != null)
                        {
                            logger.Info("taillight2 object found.");
                            // Ensure the taillight2 object is initially off
                            ...
                            {
                                logger.Debug("Renderer component is not found on taillight2 object.");
                            }
                        }
                        else
                        {
                            logger.Debug("taillight2 object not found under Z.");
                        }
                        ...
                        if (ControllerX != null)
                        {
                            logger.Info("ControllerX object found.");
                            // Store initial position and rotation of the stick
                            ControllerXStartPosition = ControllerX.transform.position; // these could cause the controller to mess up
                            ControllerXStartRotation = ControllerX.transform.rotation;

                            // Find ControllerY under ControllerX
                            ControllerY = ControllerX.Find("ControllerY");
                            if (ControllerY != null)
                            {
                                logger.Info("ControllerY object found.");
                                // Store initial position and rotation of the stick
                                ControllerYStartPosition = ControllerY.transform.position;
                                ControllerYStartRotation = ControllerY.transform.rotation;

                                // Find ControllerZ under ControllerY
                                ControllerZ = ControllerY.Find("ControllerZ");
                                if (ControllerZ != null)
                                {
                                    logger.Info("ControllerZ object found.");
                                    // Store initial position and rotation of the stick
                                    ControllerZStartPosition = ControllerZ.transform.position;
                                    ControllerZStartRotation = ControllerZ.transform.rotation;
                                }
                                else
                                {
                                    logger.Error("ControllerZ object not found under ControllerY!");
                                }
                            }
                            else
                            {
                                logger.Error("ControllerY object not found under ControllerX!");
                            }
                        }
                        else
                        {
                            logger.Error("ControllerX object not found under Z!");
                        }
                        // Gets all Light components in the target object and its children
                        Light[] allLights = ZObject.GetComponentsInChildren<Light>();
            ...
            hazzardEmissiveObjects[0] = ZObject.Find("hazard")?.GetComponent<Renderer>();
            hazzardEmissiveObjects[1] = ZObject.Find("hazardl")?.GetComponent<Renderer>();
            hazzardEmissiveObjects[2] = ZObject.Find("hazardr")?.GetComponent<Renderer>();
        ...
        void HandleInput(ref bool inputDetected, ref bool throttleDetected) // Pass by reference
        {
            ...
            // Handle X rotation for YObject and ControllerX (Down Arrow or primaryThumbstick.y > 0)
            ...
            if ((Input.GetKey(KeyCode.DownArrow) || primaryThumbstick.y > 0))
            {
                ...
                if (currentControllerRotationX > -controllerrotationLimitX)
                {
                    ...
                    ControllerX.Rotate(controllerRotateX, 0, 0);
                    currentControllerRotationX -= controllerRotateX;
                    inputDetected = true;
                }
            }

            // Handle X rotation for YObject and ControllerX (Up Arrow or primaryThumbstick.y < 0)
            ...
            if (primaryThumbstick.x < 0 && currentRotationY < rotationLimitY) // Note the change in condition
            {
                ...
                YObject.Rotate(0, rotateY, 0);  // Rotate Y in the opposite direction
        ...
        void ToggleBrightness1(bool isBright)
        {
            if (brakelight1!= null)
            {
                brakelight1.intensity = isBright ? brightIntensity : dimIntensity;
                // logger.Info($"{brakelight1.name} light set to {(isBright ? "bright" : "dim")}.");
            }
            else
            {
                logger.Debug("brakelight1 light component is not found.");
            }
        }
        ...
        void ToggleBrightness2(bool isBright)
        {
            if (brakelight2 != null)
            {
                brakelight2.intensity = isBright ? brightIntensity : dimIntensity;
                // logger.Info($"{brakelight2.name} light set to {(isBright ? "bright" : "dim")}.");
            }
            else
            {
                logger.Debug("brakelight2 light component is not found.");
            }
        }
        ...
        // Method to toggle the brakelight1 light
        void ToggleLight1(bool isActive)
        {
            if (brakelight1 != null)
            {
                brakelight1.enabled = isActive;
                //          logger.Info($"{brakelight1.name} light turned {(isActive ? "on" : "off")}.");
            }
            else
            {
                logger.Debug("brakelight1 light component is not found.");
        ...
        // Method to toggle the brakelight2 light
        void ToggleLight2(bool isActive)
        {
            if (brakelight2 != null)
            {
                brakelight2.enabled = isActive;
                //       logger.Info($"{brakelight2.name} light turned {(isActive ? "on" : "off")}.");
            }
            else
            {
                logger.Debug("brakelight2 light component is not found.");
        ...
        void ToggleBrakeEmissive(bool isActive)
        {
            if (taillight1Object != null)
            {
                ...
                if (renderer != null)
                {
                    ...
                    //    logger.Info($"taillight1Object object emission turned {(isActive ? "on" : "off")}.");
                }
                else
                {
                    logger.Debug("Renderer component is not found on taillight1Object object.");
                }
            }
            else
            {
                logger.Debug("taillight1Object object is not assigned.");
            }
            if (taillight2Object != null)
            {
                Renderer renderer = taillight2Object.GetComponent<Renderer>();
                if (renderer != null)
                {
                    ...
                    //           logger.Info($"taillight2Object object emission turned {(isActive ? "on" : "off")}.");
                }
                else
                {
                    logger.Debug("Renderer component is not found on taillight2Object object.");
                }
            }
            else
            {
                logger.Debug("taillight2Object object is not assigned.");
            }
        }
        ...
        void LogMissingObject(Renderer[] emissiveObjects, string arrayName)
        {
            for (int i = 0; i < emissiveObjects.Length; i++)
            {
                ...
                {
                    logger.Debug($"{arrayName} object at index {i} not found under Z.");
                }
            }
        }
        ...
        IEnumerator FrontHazzardPattern()
        {
            while (true)
            {
                ToggleEmissive(hazzardEmissiveObjects[0], true);
        ...
        public void StartAttractPattern()
        {
            ...
            frontCoroutine = StartCoroutine(FrontHazzardPattern());
