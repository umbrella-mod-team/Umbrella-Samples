...
namespace WIGUx.Modules.harleyMotionSim
{
	public class harleyMotionSimController : MonoBehaviour
	{
        ...
        private Transform rfootemissiveObject; // Reference to the Fire right light emissive
        private Transform lfootemissiveObject; // Reference to the Fire right light emissive
        ...
        private void CycleEmissiveColors()
        {
            ...
            Transform[] emissives = { lfootemissiveObject, rfootemissiveObject };
        ...
        // Align the player head so its world height matches the rig (cockpitCam / vrCam)
			...
			if (XObject != null)
			{
				...
				if (YObject != null)
				{
					...
					if (ZObject != null)
					{
                        ...
                        // Find rfootemissive object under Z
                        rfootemissiveObject = ZObject.Find("rfootemissive");
						if (rfootemissiveObject != null)
						{
							logger.Debug($"{gameObject.name} rfootemissive object found.");
                            // Ensure the rfootemissive object is initially off
                            Renderer renderer = rfootemissiveObject.GetComponent<Renderer>();
							if (renderer != null)
							{
								renderer.material.DisableKeyword("_EMISSION");
							}
						...
						{
							logger.Debug($"{gameObject.name} rfootemissive object not found.");
						}
                        // Find lfootemissive object under Z
                        lfootemissiveObject = ZObject.Find("lfootemissive");
                        if (lfootemissiveObject != null)
                        {
                            logger.Debug($"{gameObject.name} lfootemissive object found.");
                            // Ensure the lfootemissive object is initially off
                            Renderer renderer = lfootemissiveObject.GetComponent<Renderer>();
                            if (renderer != null)
                            {
                                renderer.material.DisableKeyword("_EMISSION");
                            }
                        ...
                        {
                            logger.Debug($"{gameObject.name} lfootemissive object not found.");
                        }
                    }
					else
					{
