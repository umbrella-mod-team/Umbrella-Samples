...
namespace WIGUx.Modules.id7MotionSim
{
    public class id7MotionSimController : MonoBehaviour
    {
        ...
        private Transform id7XObject; // Reference to the main X object
        private Transform id7YObject; // Reference to the main Y object
        private Transform id7ZObject; // Reference to the main Z object
        private GameObject cockpitCam;    // Reference to the cockpit camera

        // Initial positions and rotations for resetting
        private Vector3 id7XStartPosition;
        private Quaternion id7XStartRotation;
        private Vector3 id7YStartPosition;
        private Quaternion id7YStartRotation;
        private Vector3 id7ZStartPosition;
        private Quaternion id7ZStartRotation;
        ...
        private Transform id7ControllerX; // Reference to the main animated controller (wheel)
        private Vector3 id7ControllerXStartPosition; // Initial controller positions and rotations for resetting
        private Quaternion id7ControllerXStartRotation; // Initial controlller positions and rotations for resetting
        private Transform id7ControllerY; // Reference to the main animated controller (wheel)
        private Vector3 id7ControllerYStartPosition; // Initial controller positions and rotations for resetting
        private Quaternion id7ControllerYStartRotation; // Initial controlller positions and rotations for resetting
        private Transform id7ControllerZ; // Reference to the main animated controller (wheel)
        private Vector3 id7ControllerZStartPosition; // Initial controller positions and rotations for resetting
        private Quaternion id7ControllerZStartRotation; // Initial controlller positions and rotations for resetting
        ...
        private Transform id7taillight1Object;
        private Transform id7taillight2Object;
        private Transform id7hazardObject;
        private Transform id7hazardlObject;
        private Transform id7hazardrObject;
        ...
        public Light id7brakelight1;
        public Light id7brakelight2;
        ...
        private readonly string[] compatibleGames = { "Initial D - Arcade Stage 7 Double Ace Cross" };
        ...
        void Start()
        {
            ...
            id7XObject = transform.Find("id7X");
            if (id7XObject != null)
            {
                logger.Info("id7X object found.");
                id7XStartPosition = id7XObject.position;
                id7XStartRotation = id7XObject.rotation;

                // Find id7Y object under id7X
                id7YObject = id7XObject.Find("id7Y");
                if (id7YObject != null)
                {
                    logger.Info("id7Y object found.");
                    id7YStartPosition = id7YObject.position;
                    id7YStartRotation = id7YObject.rotation;

                    // Find id7Z object under id7Y
                    id7ZObject = id7YObject.Find("id7Z");
                    if (id7ZObject != null)
                    {
                        logger.Info("id7Z object found.");
                        id7ZStartPosition = id7ZObject.position;
                        id7ZStartRotation = id7ZObject.rotation;

                        // Find id7hazard object under id7Z
                        id7hazardObject = id7ZObject.Find("id7hazard");
                        if (id7hazardObject != null)
                        {
                            logger.Info("id7hazard object found.");

                            // Ensure the id7hazard object's emission is initially off for all materials
                            Renderer renderer = id7hazardObject.GetComponent<Renderer>();
                            if (renderer != null)
                            {
                            ...
                            {
                                logger.Debug("Renderer component is not found on id7hazard object.");
                            }
                        }
                        else
                        {
                            logger.Debug("id7hazard object not found under id7Z.");
                        }


                        // Find id7ControllerX under id7Z
                        id7ControllerX = id7ZObject.Find("id7ControllerX");
                        if (id7ControllerX != null)
                        {
                            logger.Info("id7ControllerX object found.");
                            // Store initial position and rotation of the stick
                            id7ControllerXStartPosition = id7ControllerX.transform.position; // these could cause the controller to mess up
                            id7ControllerXStartRotation = id7ControllerX.transform.rotation;

                            // Find id7ControllerY under id7ControllerX
                            id7ControllerY = id7ControllerX.Find("id7ControllerY");
                            if (id7ControllerY != null)
                            {
                                logger.Info("id7ControllerY object found.");
                                // Store initial position and rotation of the stick
                                id7ControllerYStartPosition = id7ControllerY.transform.position;
                                id7ControllerYStartRotation = id7ControllerY.transform.rotation;

                                // Find id7ControllerZ under id7ControllerY
                                id7ControllerZ = id7ControllerY.Find("id7ControllerZ");
                                if (id7ControllerZ != null)
                                {
                                    logger.Info("id7ControllerZ object found.");
                                    // Store initial position and rotation of the stick
                                    id7ControllerZStartPosition = id7ControllerZ.transform.position;
                                    id7ControllerZStartRotation = id7ControllerZ.transform.rotation;
                                }
                                else
                                {
                                    logger.Error("id7ControllerZ object not found under id7ControllerY!");
                                }
                            }
                            else
                            {
                                logger.Error("id7ControllerY object not found under id7ControllerX!");
                            }
                        }
                        else
                        {
                            logger.Error("id7ControllerX object not found under id7Z!");
                        }
                        // Gets all Light components in the target object and its children
                        Light[] allLights = id7ZObject.GetComponentsInChildren<Light>();

                        // Log the names of the objects containing the Light components and filter out unwanted lights
                        foreach (Light light in allLights)
                        {
                            if (light.gameObject.name == "id7brakelight1")
                            {
                                id7brakelight1 = light;
                                logger.Info("Included Light found in object: " + light.gameObject.name);
                            }
                            else if (light.gameObject.name == "id7brakelight2")
                            {
                                id7brakelight2 = light;
                                logger.Info("Included Light found in object: " + light.gameObject.name);
                            }
                        ...
                        cockpitCam = id7ZObject.Find("eyes/cockpitcam")?.gameObject;
                        if (cockpitCam != null)
                        {
                        ...
                        {
                            logger.Error("Cockpitcam object not found under id7Z!");
                        }
                    }
                    else
                    {
                        logger.Error("id7Z object not found under id7Y!");
                    }
                }
                else
                {
                    logger.Error("id7Y object not found under id7X!");
                }
            }
            else
            {
                logger.Error("id7X object not found!");
            }
            hazzardEmissiveObjects = new Renderer[1];
            //leftEmissiveObjects = new Renderer[1];
            //rightEmissiveObjects = new Renderer[1];

            hazzardEmissiveObjects[0] = id7ZObject.Find("id7hazard")?.GetComponent<Renderer>();
            //hazzardEmissiveObjects[1] = id7ZObject.Find("id7hazardl")?.GetComponent<Renderer>();
            // hazzardEmissiveObjects[2] = id7ZObject.Find("id7hazardr")?.GetComponent<Renderer>();


            //leftEmissiveObjects[0] = id7ZObject.Find("left1")?.GetComponent<Renderer>();
            //leftEmissiveObjects[1] = id7ZObject.Find("left2")?.GetComponent<Renderer>();
            // leftEmissiveObjects[2] = id7ZObject.Find("left3")?.GetComponent<Renderer>();

            //rightEmissiveObjects[0] = id7ZObject.Find("right1")?.GetComponent<Renderer>();
            //rightEmissiveObjects[1] = id7ZObject.Find("right2")?.GetComponent<Renderer>();
            //rightEmissiveObjects[2] = id7ZObject.Find("right3")?.GetComponent<Renderer>();
            // Set lights to start
        ...
        void StartFocusMode()
        {
            ...
            logger.Info("Compatible Rom Dectected, Starting Initial D - Arcade Stage 7 Double Ace Cross...");
            logger.Info("Initial D 7 Motion Sim starting...Takeshi Nakazato your car is ready");
            logger.Info("Nissan Skyline GT-R ready to Race!...");
        ...
        void EndFocusMode()
        {
            ...
            // Reset id7X to initial positions and rotations
            if (id7XObject != null)
            {
                id7XObject.position = id7XStartPosition;
                id7XObject.rotation = id7XStartRotation;
            }

            // Reset id7Y object to initial position and rotation
            if (id7YObject != null)
            {
                id7YObject.position = id7YStartPosition;
                id7YObject.rotation = id7YStartRotation;
            }
            // Reset id7Z object to initial position and rotation
            if (id7ZObject != null)
            {
                id7ZObject.position = id7ZStartPosition;
                id7ZObject.rotation = id7ZStartRotation;
            }
        ...
        void HandleInput(ref bool inputDetected, ref bool throttleDetected) // Pass by reference
        {
            ...
            */
            ...
            // Handle X rotation for id7YObject and id7ControllerX (Down Arrow or primaryThumbstick.y > 0)
            ...
            if ((Input.GetKey(KeyCode.DownArrow) || primaryThumbstick.y > 0))
            {
                ...
                if (currentControllerRotationX > -controllerrotationLimitX)
                {
                    ...
                    id7ControllerX.Rotate(controllerRotateX, 0, 0);
                    currentControllerRotationX -= controllerRotateX;
                    inputDetected = true;
                }
            }

            // Handle X rotation for id7YObject and id7ControllerX (Up Arrow or primaryThumbstick.y < 0)
            ...
            */
            // Handle Z rotation for id7XObject and id7ControllerZ (Down Arrow or primaryThumbstick.y < 0)
            ...
            if ((Input.GetKey(KeyCode.LeftArrow) || primaryThumbstick.x < 0))
            {
                ...
                if (currentControllerRotationZ > -controllerrotationLimitZ)
                {
                    ...
                    id7ControllerZ.Rotate(0, 0, controllerRotateZ);
                    currentControllerRotationZ -= controllerRotateZ;
                    inputDetected = true;
                }
            }

            // Handle Z rotation for id7XObject and id7ControllerZ (Up Arrow or primaryThumbstick.y > 0)
            ...
            if (primaryThumbstick.x < 0 && currentRotationY < rotationLimitY) // Note the change in condition
            {
                ...
                id7YObject.Rotate(0, rotateY, 0);  // Rotate Y in the opposite direction
        ...
        void CenterRotation()
        {
            ...
            if (currentRotationY > 0)
            {
                ...
                id7YObject.Rotate(0, unrotateY, 0);
            ...
            if (currentRotationZ > 0)
            {
                ...
                id7ZObject.Rotate(0, 0, unrotateZ);
            ...
            if (currentControllerRotationY > 0)
            {
                ...
                id7ControllerY.Rotate(0, unrotateY, 0);   // Rotating to reduce the rotation
            ...
            if (currentControllerRotationX > 0)
            {
                ...
                id7ControllerX.Rotate(unrotateX, 0, 0);   // Rotating to reduce the rotation
            ...
            if (currentControllerRotationZ > 0)
            {
                ...
                id7ControllerZ.Rotate(0, 0, unrotateZ);   // Rotating to reduce the rotation
        ...
        {
            if (id7brakelight1!= null)
            {
                id7brakelight1.intensity = isBright ? brightIntensity : dimIntensity;
                // logger.Info($"{id7brakelight1.name} light set to {(isBright ? "bright" : "dim")}.");
            }
            else
            {
                logger.Debug("id7brakelight1 light component is not found.");
            }
        }
        ...
        {
            if (id7brakelight2 != null)
            {
                id7brakelight2.intensity = isBright ? brightIntensity : dimIntensity;
                // logger.Info($"{id7brakelight2.name} light set to {(isBright ? "bright" : "dim")}.");
            }
            else
            {
                logger.Debug("id7brakelight2 light component is not found.");
            }
        }
        ...
        // Method to toggle the id7brakelight1 light
        void ToggleLight1(bool isActive)
        {
            if (id7brakelight1 != null)
            {
                id7brakelight1.enabled = isActive;
                //          logger.Info($"{id7brakelight1.name} light turned {(isActive ? "on" : "off")}.");
            }
            else
            {
                logger.Debug("id7brakelight1 light component is not found.");
        ...
        // Method to toggle the id7brakelight2 light
        void ToggleLight2(bool isActive)
        {
            if (id7brakelight2 != null)
            {
                id7brakelight2.enabled = isActive;
                //       logger.Info($"{id7brakelight2.name} light turned {(isActive ? "on" : "off")}.");
            }
            else
            {
                logger.Debug("id7brakelight2 light component is not found.");
        ...
        void ToggleBrakeEmissive(bool isActive)
        {
            if (id7hazardObject != null)
            {
                ...
                if (renderer != null)
                {
                    ...
                    // logger.Info($"id7hazardObject object emission turned {(isActive ? "on" : "off")}.");
                }
                else
                {
                    logger.Debug("Renderer component is not found on id7hazardObject object.");
                }
            }
            else
            {
                logger.Debug("id7hazardObject object is not assigned.");
            }
        }
        ...
        void LogMissingObject(Renderer[] emissiveObjects, string arrayName)
        {
            for (int i = 0; i < emissiveObjects.Length; i++)
            {
                ...
                {
                    logger.Debug($"{arrayName} object at index {i} not found under id7Z.");
                }
            }
        }

        IEnumerator id7FrontAttractPattern()
        {
            while (true)
            {
        ...
        IEnumerator id7FrontHazzardPattern()
        {
            while (true)
            {
                ToggleEmissive(hazzardEmissiveObjects[0], true);
        ...
        public void StartAttractPattern()
        {
            ...
            frontCoroutine = StartCoroutine(id7FrontHazzardPattern());
        ...
        public void StartDangerPattern()
        {
            ...
            frontCoroutine = StartCoroutine(id7FrontAttractPattern());
