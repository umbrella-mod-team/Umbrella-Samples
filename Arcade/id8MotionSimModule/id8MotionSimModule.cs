...
namespace WIGUx.Modules.id8MotionSim
{
    public class id8MotionSimController : MonoBehaviour
    {
        ...
        private Transform id8XObject; // Reference to the main X object
        private Transform id8YObject; // Reference to the main Y object
        private Transform id8ZObject; // Reference to the main Z object
        private GameObject cockpitCam;    // Reference to the cockpit camera

        // Initial positions and rotations for resetting
        private Vector3 id8XStartPosition;
        private Quaternion id8XStartRotation;
        private Vector3 id8YStartPosition;
        private Quaternion id8YStartRotation;
        private Vector3 id8ZStartPosition;
        private Quaternion id8ZStartRotation;
        ...
        private Transform id8ControllerX; // Reference to the main animated controller (wheel)
        private Vector3 id8ControllerXStartPosition; // Initial controller positions and rotations for resetting
        private Quaternion id8ControllerXStartRotation; // Initial controlller positions and rotations for resetting
        private Transform id8ControllerY; // Reference to the main animated controller (wheel)
        private Vector3 id8ControllerYStartPosition; // Initial controller positions and rotations for resetting
        private Quaternion id8ControllerYStartRotation; // Initial controlller positions and rotations for resetting
        private Transform id8ControllerZ; // Reference to the main animated controller (wheel)
        private Vector3 id8ControllerZStartPosition; // Initial controller positions and rotations for resetting
        private Quaternion id8ControllerZStartRotation; // Initial controlller positions and rotations for resetting
        ...
        private Transform id8taillight1Object;
        private Transform id8taillight2Object;
        private Transform id8hazardObject;
        private Transform id8hazardlObject;
        private Transform id8hazardrObject;
        ...
        public Light id8brakelight1;
        public Light id8brakelight2;
        ...
        private readonly string[] compatibleGames = { "Initial D - Arcade Stage 8 Infinity" };
        ...
        void Start()
        {
            ...
            id8XObject = transform.Find("id8X");
            if (id8XObject != null)
            {
                logger.Info("id8X object found.");
                id8XStartPosition = id8XObject.position;
                id8XStartRotation = id8XObject.rotation;

                // Find id8Y object under id8X
                id8YObject = id8XObject.Find("id8Y");
                if (id8YObject != null)
                {
                    logger.Info("id8Y object found.");
                    id8YStartPosition = id8YObject.position;
                    id8YStartRotation = id8YObject.rotation;

                    // Find id8Z object under id8Y
                    id8ZObject = id8YObject.Find("id8Z");
                    if (id8ZObject != null)
                    {
                        logger.Info("id8Z object found.");
                        id8ZStartPosition = id8ZObject.position;
                        id8ZStartRotation = id8ZObject.rotation;

                        // Find id8hazard object under id8Z
                        id8hazardObject = id8ZObject.Find("id8hazard");
                        if (id8hazardObject != null)
                        {
                            logger.Info("id8hazard object found.");

                            // Ensure the id8hazard object's emission is initially off for all materials
                            Renderer renderer = id8hazardObject.GetComponent<Renderer>();
                            if (renderer != null)
                            {
                            ...
                            {
                                logger.Debug("Renderer component is not found on id8hazard object.");
                            }
                        }
                        else
                        {
                            logger.Debug("id8hazard object not found under id8Z.");
                        }


                        // Find id8ControllerX under id8Z
                        id8ControllerX = id8ZObject.Find("id8ControllerX");
                        if (id8ControllerX != null)
                        {
                            logger.Info("id8ControllerX object found.");
                            // Store initial position and rotation of the stick
                            id8ControllerXStartPosition = id8ControllerX.transform.position; // these could cause the controller to mess up
                            id8ControllerXStartRotation = id8ControllerX.transform.rotation;

                            // Find id8ControllerY under id8ControllerX
                            id8ControllerY = id8ControllerX.Find("id8ControllerY");
                            if (id8ControllerY != null)
                            {
                                logger.Info("id8ControllerY object found.");
                                // Store initial position and rotation of the stick
                                id8ControllerYStartPosition = id8ControllerY.transform.position;
                                id8ControllerYStartRotation = id8ControllerY.transform.rotation;

                                // Find id8ControllerZ under id8ControllerY
                                id8ControllerZ = id8ControllerY.Find("id8ControllerZ");
                                if (id8ControllerZ != null)
                                {
                                    logger.Info("id8ControllerZ object found.");
                                    // Store initial position and rotation of the stick
                                    id8ControllerZStartPosition = id8ControllerZ.transform.position;
                                    id8ControllerZStartRotation = id8ControllerZ.transform.rotation;
                                }
                                else
                                {
                                    logger.Error("id8ControllerZ object not found under id8ControllerY!");
                                }
                            }
                            else
                            {
                                logger.Error("id8ControllerY object not found under id8ControllerX!");
                            }
                        }
                        else
                        {
                            logger.Error("id8ControllerX object not found under id8Z!");
                        }
                        // Gets all Light components in the target object and its children
                        Light[] allLights = id8ZObject.GetComponentsInChildren<Light>();

                        // Log the names of the objects containing the Light components and filter out unwanted lights
                        foreach (Light light in allLights)
                        {
                            if (light.gameObject.name == "id8brakelight1")
                            {
                                id8brakelight1 = light;
                                logger.Info("Included Light found in object: " + light.gameObject.name);
                            }
                            else if (light.gameObject.name == "id8brakelight2")
                            {
                                id8brakelight2 = light;
                                logger.Info("Included Light found in object: " + light.gameObject.name);
                            }
                        ...
                        cockpitCam = id8ZObject.Find("eyes/cockpitcam")?.gameObject;
                        if (cockpitCam != null)
                        {
                        ...
                        {
                            logger.Error("Cockpitcam object not found under id8Z!");
                        }
                    }
                    else
                    {
                        logger.Error("id8Z object not found under id8Y!");
                    }
                }
                else
                {
                    logger.Error("id8Y object not found under id8X!");
                }
            }
            else
            {
                logger.Error("id8X object not found!");
            }
            hazzardEmissiveObjects = new Renderer[1];
            //leftEmissiveObjects = new Renderer[1];
            //rightEmissiveObjects = new Renderer[1];

            hazzardEmissiveObjects[0] = id8ZObject.Find("id8hazard")?.GetComponent<Renderer>();
            //hazzardEmissiveObjects[1] = id8ZObject.Find("id8hazardl")?.GetComponent<Renderer>();
            // hazzardEmissiveObjects[2] = id8ZObject.Find("id8hazardr")?.GetComponent<Renderer>();
  
            //leftEmissiveObjects[0] = id8ZObject.Find("left1")?.GetComponent<Renderer>();
            //leftEmissiveObjects[1] = id8ZObject.Find("left2")?.GetComponent<Renderer>();
            // leftEmissiveObjects[2] = id8ZObject.Find("left3")?.GetComponent<Renderer>();

            //rightEmissiveObjects[0] = id8ZObject.Find("right1")?.GetComponent<Renderer>();
            //rightEmissiveObjects[1] = id8ZObject.Find("right2")?.GetComponent<Renderer>();
            //rightEmissiveObjects[2] = id8ZObject.Find("right3")?.GetComponent<Renderer>();
            // Set lights to start
        ...
        void StartFocusMode()
        {
            ...
            logger.Info("Compatible Rom Dectected, Starting Initial D - Arcade Stage 8 Infinity...");
            logger.Info("Initial D 7 Motion Sim starting...Shingo Shoji of the Night Kids racing");
            logger.Info("Honda Civic SiR-II ready to Race!...");
        ...
        void EndFocusMode()
        {
            ...
            // Reset id8X to initial positions and rotations
            if (id8XObject != null)
            {
                id8XObject.position = id8XStartPosition;
                id8XObject.rotation = id8XStartRotation;
            }

            // Reset id8Y object to initial position and rotation
            if (id8YObject != null)
            {
                id8YObject.position = id8YStartPosition;
                id8YObject.rotation = id8YStartRotation;
            }
            // Reset id8Z object to initial position and rotation
            if (id8ZObject != null)
            {
                id8ZObject.position = id8ZStartPosition;
                id8ZObject.rotation = id8ZStartRotation;
            }
        ...
        void HandleInput(ref bool inputDetected, ref bool throttleDetected) // Pass by reference
        {
            ...
            */
            ...
            // Handle X rotation for id8YObject and id8ControllerX (Down Arrow or primaryThumbstick.y > 0)
            ...
            if ((Input.GetKey(KeyCode.DownArrow) || primaryThumbstick.y > 0))
            {
                ...
                if (currentControllerRotationX > -controllerrotationLimitX)
                {
                    ...
                    id8ControllerX.Rotate(controllerRotateX, 0, 0);
                    currentControllerRotationX -= controllerRotateX;
                    inputDetected = true;
                }
            }

            // Handle X rotation for id8YObject and id8ControllerX (Up Arrow or primaryThumbstick.y < 0)
            ...
            */
            // Handle Z rotation for id8XObject and id8ControllerZ (Down Arrow or primaryThumbstick.y < 0)
            ...
            if ((Input.GetKey(KeyCode.LeftArrow) || primaryThumbstick.x < 0))
            {
                ...
                if (currentControllerRotationZ > -controllerrotationLimitZ)
                {
                    ...
                    id8ControllerZ.Rotate(0, 0, controllerRotateZ);
                    currentControllerRotationZ -= controllerRotateZ;
                    inputDetected = true;
                }
            }

            // Handle Z rotation for id8XObject and id8ControllerZ (Up Arrow or primaryThumbstick.y > 0)
            ...
            if (primaryThumbstick.x < 0 && currentRotationY < rotationLimitY) // Note the change in condition
            {
                ...
                id8YObject.Rotate(0, rotateY, 0);  // Rotate Y in the opposite direction
        ...
        void CenterThrottle()
        {
            ...
            if (currentRotationX > 0)
            {
                ...
                id8XObject.Rotate(unrotateX, 0, 0);
        ...
        void CenterRotation()
        {
            ...
            if (currentRotationY > 0)
            {
                ...
                id8YObject.Rotate(0, unrotateY, 0);
            ...
            if (currentRotationZ > 0)
            {
                ...
                id8ZObject.Rotate(0, 0, unrotateZ);
            ...
            if (currentControllerRotationY > 0)
            {
                ...
                id8ControllerY.Rotate(0, unrotateY, 0);   // Rotating to reduce the rotation
            ...
            if (currentControllerRotationX > 0)
            {
                ...
                id8ControllerX.Rotate(unrotateX, 0, 0);   // Rotating to reduce the rotation
            ...
            if (currentControllerRotationZ > 0)
            {
                ...
                id8ControllerZ.Rotate(0, 0, unrotateZ);   // Rotating to reduce the rotation
        ...
        {
            if (id8brakelight1!= null)
            {
                id8brakelight1.intensity = isBright ? brightIntensity : dimIntensity;
                // logger.Info($"{id8brakelight1.name} light set to {(isBright ? "bright" : "dim")}.");
            }
            else
            {
                logger.Debug("id8brakelight1 light component is not found.");
            }
        }
        ...
        {
            if (id8brakelight2 != null)
            {
                id8brakelight2.intensity = isBright ? brightIntensity : dimIntensity;
                // logger.Info($"{id8brakelight2.name} light set to {(isBright ? "bright" : "dim")}.");
            }
            else
            {
                logger.Debug("id8brakelight2 light component is not found.");
            }
        }
        ...
        // Method to toggle the id8brakelight1 light
        void ToggleLight1(bool isActive)
        {
            if (id8brakelight1 != null)
            {
                id8brakelight1.enabled = isActive;
                //          logger.Info($"{id8brakelight1.name} light turned {(isActive ? "on" : "off")}.");
            }
            else
            {
                logger.Debug("id8brakelight1 light component is not found.");
        ...
        // Method to toggle the id8brakelight2 light
        void ToggleLight2(bool isActive)
        {
            if (id8brakelight2 != null)
            {
                id8brakelight2.enabled = isActive;
                //       logger.Info($"{id8brakelight2.name} light turned {(isActive ? "on" : "off")}.");
            }
            else
            {
                logger.Debug("id8brakelight2 light component is not found.");
        ...
        void ToggleBrakeEmissive(bool isActive)
        {
            if (id8hazardObject != null)
            {
                ...
                if (renderer != null)
                {
                    ...
                    // logger.Info($"id8hazardObject object emission turned {(isActive ? "on" : "off")}.");
                }
                else
                {
                    logger.Debug("Renderer component is not found on id8hazardObject object.");
                }
            }
            else
            {
                logger.Debug("id8hazardObject object is not assigned.");
            }
        }
        ...
        void LogMissingObject(Renderer[] emissiveObjects, string arrayName)
        {
            for (int i = 0; i < emissiveObjects.Length; i++)
            {
                ...
                {
                    logger.Debug($"{arrayName} object at index {i} not found under id8Z.");
                }
            }
        }

        IEnumerator id8FrontAttractPattern()
        {
            while (true)
            {
        ...
        IEnumerator id8FrontHazzardPattern()
        {
            while (true)
            {
                ToggleEmissive(hazzardEmissiveObjects[0], true);
        ...
        public void StartAttractPattern()
        {
            ...
            frontCoroutine = StartCoroutine(id8FrontHazzardPattern());
        ...
        public void StartDangerPattern()
        {
            ...
            frontCoroutine = StartCoroutine(id8FrontAttractPattern());
