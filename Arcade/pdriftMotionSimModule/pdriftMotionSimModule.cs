...
namespace WIGUx.Modules.pdriftMotionSim
{
    public class pdriftMotionSimController : MonoBehaviour
    {
        ...
        [Header("Timers and States")]  // Store last states and timer
        ...
        void StartFocusMode()
        {
            ...
            if (cockpitCollider != null && inside)
            {
                ...
                if (playerVRSetup != null)
                {
                    ...
                    logger.Debug("Sega Power Drift Deluxe Motion Sim starting...");
                    logger.Debug("Ready to get that Lean On?");
                    isRiding = true; // Set riding state to true
                }
            }
            else
        ...
        void InitializeLights()
        {
            ...
            foreach (var light in lights)
            {
                switch (light.gameObject.name)
                {
                    case "start_lamp":
                        start_lamp = light;
                        logger.Debug($"Included Light: {light.gameObject.name}");
                        break;
                    ...
                    default:
                        logger.Debug($"Excluded Light: {light.gameObject.name}");
                        break;
                }
            }
        }
