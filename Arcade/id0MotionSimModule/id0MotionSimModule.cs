...
namespace WIGUx.Modules.id0MotionSim
{
    public class id0MotionSimController : MonoBehaviour
    {
        ...
        private Transform id0XObject; // Reference to the main X object
        private Transform id0YObject; // Reference to the main Y object
        private Transform id0ZObject; // Reference to the main Z object
        private GameObject cockpitCam;    // Reference to the cockpit camera

        // Initial positions and rotations for resetting
        private Vector3 id0XStartPosition;
        private Quaternion id0XStartRotation;
        private Vector3 id0YStartPosition;
        private Quaternion id0YStartRotation;
        private Vector3 id0ZStartPosition;
        private Quaternion id0ZStartRotation;
        ...
        private Transform id0ControllerX; // Reference to the main animated controller (wheel)
        private Vector3 id0ControllerXStartPosition; // Initial controller positions and rotations for resetting
        private Quaternion id0ControllerXStartRotation; // Initial controlller positions and rotations for resetting
        private Transform id0ControllerY; // Reference to the main animated controller (wheel)
        private Vector3 id0ControllerYStartPosition; // Initial controller positions and rotations for resetting
        private Quaternion id0ControllerYStartRotation; // Initial controlller positions and rotations for resetting
        private Transform id0ControllerZ; // Reference to the main animated controller (wheel)
        private Vector3 id0ControllerZStartPosition; // Initial controller positions and rotations for resetting
        private Quaternion id0ControllerZStartRotation; // Initial controlller positions and rotations for resetting
        ...
        private Transform id0taillight1Object;
        private Transform id0taillight2Object;
        private Transform id0hazardObject;
        private Transform id0hazardlObject;
        private Transform id0hazardrObject;
        ...
        public Light id0brakelight1;
        public Light id0brakelight2;
        ...
        private readonly string[] compatibleGames = { "Initial D - Arcade Stage Zero" };
        ...
        void Start()
        {
            ...
            id0XObject = transform.Find("id0X");
            if (id0XObject != null)
            {
                logger.Info("id0X object found.");
                id0XStartPosition = id0XObject.position;
                id0XStartRotation = id0XObject.rotation;

                // Find id0Y object under id0X
                id0YObject = id0XObject.Find("id0Y");
                if (id0YObject != null)
                {
                    logger.Info("id0Y object found.");
                    id0YStartPosition = id0YObject.position;
                    id0YStartRotation = id0YObject.rotation;

                    // Find id0Z object under id0Y
                    id0ZObject = id0YObject.Find("id0Z");
                    if (id0ZObject != null)
                    {
                        logger.Info("id0Z object found.");
                        id0ZStartPosition = id0ZObject.position;
                        id0ZStartRotation = id0ZObject.rotation;

                        // Find id0hazard object under id0Z
                        id0hazardObject = id0ZObject.Find("id0hazard");
                        if (id0hazardObject != null)
                        {
                            logger.Info("id0hazard object found.");

                            // Ensure the id0hazard object's emission is initially off for all materials
                            Renderer renderer = id0hazardObject.GetComponent<Renderer>();
                            if (renderer != null)
                            {
                            ...
                            {
                                logger.Debug("Renderer component is not found on id0hazard object.");
                            }
                        }
                        else
                        {
                            logger.Debug("id0hazard object not found under id0Z.");
                        }

                        // Find id0ControllerX under id0Z
                        id0ControllerX = id0ZObject.Find("id0ControllerX");
                        if (id0ControllerX != null)
                        {
                            logger.Info("id0ControllerX object found.");
                            // Store initial position and rotation of the stick
                            id0ControllerXStartPosition = id0ControllerX.transform.position; // these could cause the controller to mess up
                            id0ControllerXStartRotation = id0ControllerX.transform.rotation;

                            // Find id0ControllerY under id0ControllerX
                            id0ControllerY = id0ControllerX.Find("id0ControllerY");
                            if (id0ControllerY != null)
                            {
                                logger.Info("id0ControllerY object found.");
                                // Store initial position and rotation of the stick
                                id0ControllerYStartPosition = id0ControllerY.transform.position;
                                id0ControllerYStartRotation = id0ControllerY.transform.rotation;

                                // Find id0ControllerZ under id0ControllerY
                                id0ControllerZ = id0ControllerY.Find("id0ControllerZ");
                                if (id0ControllerZ != null)
                                {
                                    logger.Info("id0ControllerZ object found.");
                                    // Store initial position and rotation of the stick
                                    id0ControllerZStartPosition = id0ControllerZ.transform.position;
                                    id0ControllerZStartRotation = id0ControllerZ.transform.rotation;
                                }
                                else
                                {
                                    logger.Error("id0ControllerZ object not found under id0ControllerY!");
                                }
                            }
                            else
                            {
                                logger.Error("id0ControllerY object not found under id0ControllerX!");
                            }
                        }
                        else
                        {
                            logger.Error("id0ControllerX object not found under id0Z!");
                        }
                        // Gets all Light components in the target object and its children
                        Light[] allLights = id0ZObject.GetComponentsInChildren<Light>();

                        // Log the names of the objects containing the Light components and filter out unwanted lights
                        foreach (Light light in allLights)
                        {
                            if (light.gameObject.name == "id0brakelight1")
                            {
                                id0brakelight1 = light;
                                logger.Info("Included Light found in object: " + light.gameObject.name);
                            }
                            else if (light.gameObject.name == "id0brakelight2")
                            {
                                id0brakelight2 = light;
                                logger.Info("Included Light found in object: " + light.gameObject.name);
                            }
                        ...
                        cockpitCam = id0ZObject.Find("eyes/cockpitcam")?.gameObject;
                        if (cockpitCam != null)
                        {
                        ...
                        {
                            logger.Error("Cockpitcam object not found under id0Z!");
                        }
                    }
                    else
                    {
                        logger.Error("id0Z object not found under id0Y!");
                    }
                }
                else
                {
                    logger.Error("id0Y object not found under id0X!");
                }
            }
            else
            {
                logger.Error("id0X object not found!");
            }
            hazzardEmissiveObjects = new Renderer[1];
            //leftEmissiveObjects = new Renderer[1];
            //rightEmissiveObjects = new Renderer[1];

            hazzardEmissiveObjects[0] = id0ZObject.Find("id0hazard")?.GetComponent<Renderer>();
            //hazzardEmissiveObjects[1] = id0ZObject.Find("id0hazardl")?.GetComponent<Renderer>();
            //hazzardEmissiveObjects[2] = id0ZObject.Find("id0hazardr")?.GetComponent<Renderer>();

            //leftEmissiveObjects[0] = id0ZObject.Find("left1")?.GetComponent<Renderer>();
            //leftEmissiveObjects[1] = id0ZObject.Find("left2")?.GetComponent<Renderer>();
            // leftEmissiveObjects[2] = id0ZObject.Find("left3")?.GetComponent<Renderer>();

            //rightEmissiveObjects[0] = id0ZObject.Find("right1")?.GetComponent<Renderer>();
            //rightEmissiveObjects[1] = id0ZObject.Find("right2")?.GetComponent<Renderer>();
            //rightEmissiveObjects[2] = id0ZObject.Find("right3")?.GetComponent<Renderer>();
            // Set lights to start
        ...
        void StartFocusMode()
        {
            ...
            logger.Info("Compatible Rom Dectected, Starting Initial D - Arcade Stage Zero...");
            logger.Info("Initial D 0 Motion Sim starting...Iketani Koichiro defend the honor of Akina");
            logger.Info("Unleash the power of the Silvia!...");
        ...
        void EndFocusMode()
        {
            ...
            // Reset id0X to initial positions and rotations
            if (id0XObject != null)
            {
                id0XObject.position = id0XStartPosition;
                id0XObject.rotation = id0XStartRotation;
            }

            // Reset id0Y object to initial position and rotation
            if (id0YObject != null)
            {
                id0YObject.position = id0YStartPosition;
                id0YObject.rotation = id0YStartRotation;
            }
            // Reset id0Z object to initial position and rotation
            if (id0ZObject != null)
            {
                id0ZObject.position = id0ZStartPosition;
                id0ZObject.rotation = id0ZStartRotation;
            }
        ...
        void HandleInput(ref bool inputDetected)
        {
            ...
            */
            ...
            // Handle X rotation for id0YObject and id0ControllerX (Down Arrow or primaryThumbstick.y > 0)
            ...
            if ((Input.GetKey(KeyCode.DownArrow) || primaryThumbstick.y > 0))
            {
                ...
                if (currentControllerRotationX > -controllerrotationLimitX)
                {
                    ...
                    id0ControllerX.Rotate(controllerRotateX, 0, 0);
                    currentControllerRotationX -= controllerRotateX;
                    inputDetected = true;
                }
            }

            // Handle X rotation for id0YObject and id0ControllerX (Up Arrow or primaryThumbstick.y < 0)
            ...
            */
            // Handle Z rotation for id0XObject and id0ControllerZ (Down Arrow or primaryThumbstick.y < 0)
            ...
            if ((Input.GetKey(KeyCode.LeftArrow) || primaryThumbstick.x < 0))
            {
                ...
                if (currentControllerRotationZ > -controllerrotationLimitZ)
                {
                    ...
                    id0ControllerZ.Rotate(0, 0, controllerRotateZ);
                    currentControllerRotationZ -= controllerRotateZ;
                    inputDetected = true;
                }
            }

            // Handle Z rotation for id0XObject and id0ControllerZ (Up Arrow or primaryThumbstick.y > 0)
            ...
            if (primaryThumbstick.x < 0 && currentRotationY < rotationLimitY) // Note the change in condition
            {
                ...
                id0YObject.Rotate(0, rotateY, 0);  // Rotate Y in the opposite direction
        ...
        void CenterRotation()
        {
            ...
            if (currentRotationX > 0)
            {
                ...
                id0XObject.Rotate(unrotateX, 0, 0);
            ...
            if (currentRotationY > 0)
            {
                ...
                id0YObject.Rotate(0, unrotateY, 0);
            ...
            if (currentRotationZ > 0)
            {
                ...
                id0ZObject.Rotate(0, 0, unrotateZ);
            ...
            if (currentControllerRotationY > 0)
            {
                ...
                id0ControllerY.Rotate(0, unrotateY, 0);   // Rotating to reduce the rotation
            ...
            if (currentControllerRotationX > 0)
            {
                ...
                id0ControllerX.Rotate(unrotateX, 0, 0);   // Rotating to reduce the rotation
            ...
            if (currentControllerRotationZ > 0)
            {
                ...
                id0ControllerZ.Rotate(0, 0, unrotateZ);   // Rotating to reduce the rotation
        ...
        {
            if (id0brakelight1!= null)
            {
                id0brakelight1.intensity = isBright ? brightIntensity : dimIntensity;
                // logger.Info($"{id0brakelight1.name} light set to {(isBright ? "bright" : "dim")}.");
            }
            else
            {
                logger.Debug("id0brakelight1 light component is not found.");
            }
        }
        ...
        {
            if (id0brakelight2 != null)
            {
                id0brakelight2.intensity = isBright ? brightIntensity : dimIntensity;
                // logger.Info($"{id0brakelight2.name} light set to {(isBright ? "bright" : "dim")}.");
            }
            else
            {
                logger.Debug("id0brakelight2 light component is not found.");
            }
        }
        ...
        // Method to toggle the id0brakelight1 light
        void ToggleLight1(bool isActive)
        {
            if (id0brakelight1 != null)
            {
                id0brakelight1.enabled = isActive;
                //          logger.Info($"{id0brakelight1.name} light turned {(isActive ? "on" : "off")}.");
            }
            else
            {
                logger.Debug("id0brakelight1 light component is not found.");
        ...
        // Method to toggle the id0brakelight2 light
        void ToggleLight2(bool isActive)
        {
            if (id0brakelight2 != null)
            {
                id0brakelight2.enabled = isActive;
                //       logger.Info($"{id0brakelight2.name} light turned {(isActive ? "on" : "off")}.");
            }
            else
            {
                logger.Debug("id0brakelight2 light component is not found.");
        ...
        void ToggleBrakeEmissive(bool isActive)
        {
            if (id0hazardObject != null)
            {
                ...
                if (renderer != null)
                {
                    ...
                    // logger.Info($"id0hazardObject object emission turned {(isActive ? "on" : "off")}.");
                }
                else
                {
                    logger.Debug("Renderer component is not found on id0hazardObject object.");
                }
            }
            else
            {
                logger.Debug("id0hazardObject object is not assigned.");
            }
        }
        ...
        void LogMissingObject(Renderer[] emissiveObjects, string arrayName)
        {
            for (int i = 0; i < emissiveObjects.Length; i++)
            {
                ...
                {
                    logger.Debug($"{arrayName} object at index {i} not found under id0Z.");
                }
            }
        }

        IEnumerator id0FrontAttractPattern()
        {
            while (true)
            {
        ...
        IEnumerator id0FrontHazzardPattern()
        {
            while (true)
            {
                ToggleEmissive(hazzardEmissiveObjects[0], true);
        ...
        public void StartAttractPattern()
        {
            ...
            frontCoroutine = StartCoroutine(id0FrontHazzardPattern());
        ...
        public void StartDangerPattern()
        {
            ...
            frontCoroutine = StartCoroutine(id0FrontAttractPattern());
