...
namespace WIGUx.Modules.idaMotionSim
{
    public class idaMotionSimController : MonoBehaviour
    {
        ...
        private Transform idaXObject; // Reference to the main X object
        private Transform idaYObject; // Reference to the main Y object
        private Transform idaZObject; // Reference to the main Z object
        private GameObject cockpitCam;    // Reference to the cockpit camera

        // Initial positions and rotations for resetting
        private Vector3 idaXStartPosition;
        private Quaternion idaXStartRotation;
        private Vector3 idaYStartPosition;
        private Quaternion idaYStartRotation;
        private Vector3 idaZStartPosition;
        private Quaternion idaZStartRotation;
        ...
        private Transform idaControllerX; // Reference to the main animated controller (wheel)
        private Vector3 idaControllerXStartPosition; // Initial controller positions and rotations for resetting
        private Quaternion idaControllerXStartRotation; // Initial controlller positions and rotations for resetting
        private Transform idaControllerY; // Reference to the main animated controller (wheel)
        private Vector3 idaControllerYStartPosition; // Initial controller positions and rotations for resetting
        private Quaternion idaControllerYStartRotation; // Initial controlller positions and rotations for resetting
        private Transform idaControllerZ; // Reference to the main animated controller (wheel)
        private Vector3 idaControllerZStartPosition; // Initial controller positions and rotations for resetting
        private Quaternion idaControllerZStartRotation; // Initial controlller positions and rotations for resetting
        ...
        private Transform idataillight1Object;
        private Transform idataillight2Object;
        private Transform idahazardObject;
        private Transform idahazardlObject;
        private Transform idahazardrObject;
        ...
        public Light idabrakelight1;
        public Light idabrakelight2;
        ...
        private readonly string[] compatibleGames = { "Initial D - The Arcade" };
        ...
        void Start()
        {
            ...
            idaXObject = transform.Find("idaX");
            if (idaXObject != null)
            {
                logger.Info("idaX object found.");
                idaXStartPosition = idaXObject.position;
                idaXStartRotation = idaXObject.rotation;

                // Find idaY object under idaX
                idaYObject = idaXObject.Find("idaY");
                if (idaYObject != null)
                {
                    logger.Info("idaY object found.");
                    idaYStartPosition = idaYObject.position;
                    idaYStartRotation = idaYObject.rotation;

                    // Find idaZ object under idaY
                    idaZObject = idaYObject.Find("idaZ");
                    if (idaZObject != null)
                    {
                        logger.Info("idaZ object found.");
                        idaZStartPosition = idaZObject.position;
                        idaZStartRotation = idaZObject.rotation;

                        // Find idahazard object under idaZ
                        idahazardObject = idaZObject.Find("idahazard");
                        if (idahazardObject != null)
                        {
                            logger.Info("idahazard object found.");

                            // Ensure the idahazard object's emission is initially off for all materials
                            Renderer renderer = idahazardObject.GetComponent<Renderer>();
                            if (renderer != null)
                            {
                            ...
                            {
                                logger.Debug("Renderer component is not found on idahazard object.");
                            }
                        }
                        else
                        {
                            logger.Debug("idahazard object not found under idaZ.");
                        }


                        // Find idaControllerX under idaZ
                        idaControllerX = idaZObject.Find("idaControllerX");
                        if (idaControllerX != null)
                        {
                            logger.Info("idaControllerX object found.");
                            // Store initial position and rotation of the stick
                            idaControllerXStartPosition = idaControllerX.transform.position; // these could cause the controller to mess up
                            idaControllerXStartRotation = idaControllerX.transform.rotation;

                            // Find idaControllerY under idaControllerX
                            idaControllerY = idaControllerX.Find("idaControllerY");
                            if (idaControllerY != null)
                            {
                                logger.Info("idaControllerY object found.");
                                // Store initial position and rotation of the stick
                                idaControllerYStartPosition = idaControllerY.transform.position;
                                idaControllerYStartRotation = idaControllerY.transform.rotation;

                                // Find idaControllerZ under idaControllerY
                                idaControllerZ = idaControllerY.Find("idaControllerZ");
                                if (idaControllerZ != null)
                                {
                                    logger.Info("idaControllerZ object found.");
                                    // Store initial position and rotation of the stick
                                    idaControllerZStartPosition = idaControllerZ.transform.position;
                                    idaControllerZStartRotation = idaControllerZ.transform.rotation;
                                }
                                else
                                {
                                    logger.Error("idaControllerZ object not found under idaControllerY!");
                                }
                            }
                            else
                            {
                                logger.Error("idaControllerY object not found under idaControllerX!");
                            }
                        }
                        else
                        {
                            logger.Error("idaControllerX object not found under idaZ!");
                        }
                        // Gets all Light components in the target object and its children
                        Light[] allLights = idaZObject.GetComponentsInChildren<Light>();

                        // Log the names of the objects containing the Light components and filter out unwanted lights
                        foreach (Light light in allLights)
                        {
                            if (light.gameObject.name == "idabrakelight1")
                            {
                                idabrakelight1 = light;
                                logger.Info("Included Light found in object: " + light.gameObject.name);
                            }
                            else if (light.gameObject.name == "idabrakelight2")
                            {
                                idabrakelight2 = light;
                                logger.Info("Included Light found in object: " + light.gameObject.name);
                            }
                        ...
                        cockpitCam = idaZObject.Find("eyes/cockpitcam")?.gameObject;
                        if (cockpitCam != null)
                        {
                        ...
                        {
                            logger.Error("Cockpitcam object not found under idaZ!");
                        }
                    }
                    else
                    {
                        logger.Error("idaZ object not found under idaY!");
                    }
                }
                else
                {
                    logger.Error("idaY object not found under idaX!");
                }
            }
            else
            {
                logger.Error("idaX object not found!");
            }
            hazzardEmissiveObjects = new Renderer[1];
            //leftEmissiveObjects = new Renderer[1];
            //rightEmissiveObjects = new Renderer[1];

            hazzardEmissiveObjects[0] = idaZObject.Find("idahazard")?.GetComponent<Renderer>();
            //hazzardEmissiveObjects[1] = idaZObject.Find("idahazardl")?.GetComponent<Renderer>();
           //hazzardEmissiveObjects[2] = idaZObject.Find("idahazardr")?.GetComponent<Renderer>();

            //leftEmissiveObjects[0] = idaZObject.Find("left1")?.GetComponent<Renderer>();
            //leftEmissiveObjects[1] = idaZObject.Find("left2")?.GetComponent<Renderer>();
            // leftEmissiveObjects[2] = idaZObject.Find("left3")?.GetComponent<Renderer>();

            //rightEmissiveObjects[0] = idaZObject.Find("right1")?.GetComponent<Renderer>();
            //rightEmissiveObjects[1] = idaZObject.Find("right2")?.GetComponent<Renderer>();
            //rightEmissiveObjects[2] = idaZObject.Find("right3")?.GetComponent<Renderer>();
            // Set lights to start
        ...
        void StartFocusMode()
        {
            ...
            logger.Info("Compatible Rom Dectected, Starting Initial D - The Arcade...");
            logger.Info("Initial D Arcade Motion Sim starting...Ryosuke Takahashi the simulations show you can't lose");
            logger.Info("Mazda RX-7 FC ready to Race! Show them the power of a Rotary...");
        ...
        void EndFocusMode()
        {
            ...
            // Reset idaX to initial positions and rotations
            if (idaXObject != null)
            {
                idaXObject.position = idaXStartPosition;
                idaXObject.rotation = idaXStartRotation;
            }

            // Reset idaY object to initial position and rotation
            if (idaYObject != null)
            {
                idaYObject.position = idaYStartPosition;
                idaYObject.rotation = idaYStartRotation;
            }
            // Reset idaZ object to initial position and rotation
            if (idaZObject != null)
            {
                idaZObject.position = idaZStartPosition;
                idaZObject.rotation = idaZStartRotation;
            }
        ...
        void HandleInput(ref bool inputDetected)
        {
            ...
            */
            ...
            // Handle X rotation for idaYObject and idaControllerX (Down Arrow or primaryThumbstick.y > 0)
            ...
            if ((Input.GetKey(KeyCode.DownArrow) || primaryThumbstick.y > 0))
            {
                ...
                if (currentControllerRotationX > -controllerrotationLimitX)
                {
                    ...
                    idaControllerX.Rotate(controllerRotateX, 0, 0);
                    currentControllerRotationX -= controllerRotateX;
                    inputDetected = true;
                }
            }

            // Handle X rotation for idaYObject and idaControllerX (Up Arrow or primaryThumbstick.y < 0)
            ...
            */
            // Handle Z rotation for idaXObject and idaControllerZ (Down Arrow or primaryThumbstick.y < 0)
            ...
            if ((Input.GetKey(KeyCode.LeftArrow) || primaryThumbstick.x < 0))
            {
                ...
                if (currentControllerRotationZ > -controllerrotationLimitZ)
                {
                    ...
                    idaControllerZ.Rotate(0, 0, controllerRotateZ);
                    currentControllerRotationZ -= controllerRotateZ;
                    inputDetected = true;
                }
            }

            // Handle Z rotation for idaXObject and idaControllerZ (Up Arrow or primaryThumbstick.y > 0)
            ...
            if (primaryThumbstick.x < 0 && currentRotationY < rotationLimitY) // Note the change in condition
            {
                ...
                idaYObject.Rotate(0, rotateY, 0);  // Rotate Y in the opposite direction
        ...
        void CenterRotation()
        {
            ...
            if (currentRotationX > 0)
            {
                ...
                idaXObject.Rotate(unrotateX, 0, 0);
            ...
            if (currentRotationY > 0)
            {
                ...
                idaYObject.Rotate(0, unrotateY, 0);
            ...
            if (currentRotationZ > 0)
            {
                ...
                idaZObject.Rotate(0, 0, unrotateZ);
            ...
            if (currentControllerRotationY > 0)
            {
                ...
                idaControllerY.Rotate(0, unrotateY, 0);   // Rotating to reduce the rotation
            ...
            if (currentControllerRotationX > 0)
            {
                ...
                idaControllerX.Rotate(unrotateX, 0, 0);   // Rotating to reduce the rotation
            ...
            if (currentControllerRotationZ > 0)
            {
                ...
                idaControllerZ.Rotate(0, 0, unrotateZ);   // Rotating to reduce the rotation
        ...
        {
            if (idabrakelight1!= null)
            {
                idabrakelight1.intensity = isBright ? brightIntensity : dimIntensity;
                // logger.Info($"{idabrakelight1.name} light set to {(isBright ? "bright" : "dim")}.");
            }
            else
            {
                logger.Debug("idabrakelight1 light component is not found.");
            }
        }
        ...
        {
            if (idabrakelight2 != null)
            {
                idabrakelight2.intensity = isBright ? brightIntensity : dimIntensity;
                // logger.Info($"{idabrakelight2.name} light set to {(isBright ? "bright" : "dim")}.");
            }
            else
            {
                logger.Debug("idabrakelight2 light component is not found.");
            }
        }
        ...
        // Method to toggle the idabrakelight1 light
        void ToggleLight1(bool isActive)
        {
            if (idabrakelight1 != null)
            {
                idabrakelight1.enabled = isActive;
                //          logger.Info($"{idabrakelight1.name} light turned {(isActive ? "on" : "off")}.");
            }
            else
            {
                logger.Debug("idabrakelight1 light component is not found.");
        ...
        // Method to toggle the idabrakelight2 light
        void ToggleLight2(bool isActive)
        {
            if (idabrakelight2 != null)
            {
                idabrakelight2.enabled = isActive;
                //       logger.Info($"{idabrakelight2.name} light turned {(isActive ? "on" : "off")}.");
            }
            else
            {
                logger.Debug("idabrakelight2 light component is not found.");
        ...
        void ToggleBrakeEmissive(bool isActive)
        {
            if (idahazardObject != null)
            {
                ...
                if (renderer != null)
                {
                    ...
                    // logger.Info($"idahazardObject object emission turned {(isActive ? "on" : "off")}.");
                }
                else
                {
                    logger.Debug("Renderer component is not found on idahazardObject object.");
                }
            }
            else
            {
                logger.Debug("idahazardObject object is not assigned.");
            }
        }
        ...
        void LogMissingObject(Renderer[] emissiveObjects, string arrayName)
        {
            for (int i = 0; i < emissiveObjects.Length; i++)
            {
                ...
                {
                    logger.Debug($"{arrayName} object at index {i} not found under idaZ.");
                }
            }
        }

        IEnumerator idaFrontAttractPattern()
        {
            while (true)
            {
        ...
        IEnumerator idaFrontHazzardPattern()
        {
            while (true)
            {
                ToggleEmissive(hazzardEmissiveObjects[0], true);
        ...
        public void StartAttractPattern()
        {
            ...
            frontCoroutine = StartCoroutine(idaFrontHazzardPattern());
        ...
        public void StartDangerPattern()
        {
            ...
            frontCoroutine = StartCoroutine(idaFrontAttractPattern());
