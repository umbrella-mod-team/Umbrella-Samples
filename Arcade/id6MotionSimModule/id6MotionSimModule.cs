...
namespace WIGUx.Modules.id6MotionSim
{
    public class id6MotionSimController : MonoBehaviour
    {
        ...
        private Transform id6XObject; // Reference to the main X object
        private Transform id6YObject; // Reference to the main Y object
        private Transform id6ZObject; // Reference to the main Z object
        private GameObject cockpitCam;    // Reference to the cockpit camera

        // Initial positions and rotations for resetting
        private Vector3 id6XStartPosition;
        private Quaternion id6XStartRotation;
        private Vector3 id6YStartPosition;
        private Quaternion id6YStartRotation;
        private Vector3 id6ZStartPosition;
        private Quaternion id6ZStartRotation;
        ...
        private Transform id6ControllerX; // Reference to the main animated controller (wheel)
        private Vector3 id6ControllerXStartPosition; // Initial controller positions and rotations for resetting
        private Quaternion id6ControllerXStartRotation; // Initial controlller positions and rotations for resetting
        private Transform id6ControllerY; // Reference to the main animated controller (wheel)
        private Vector3 id6ControllerYStartPosition; // Initial controller positions and rotations for resetting
        private Quaternion id6ControllerYStartRotation; // Initial controlller positions and rotations for resetting
        private Transform id6ControllerZ; // Reference to the main animated controller (wheel)
        private Vector3 id6ControllerZStartPosition; // Initial controller positions and rotations for resetting
        private Quaternion id6ControllerZStartRotation; // Initial controlller positions and rotations for resetting
        ...
        private Transform id6taillight1Object;
        private Transform id6taillight2Object;
        private Transform id6hazardlObject;
        private Transform id6hazardrObject;
        ...
        public Light id6brakelight1;
        public Light id6brakelight2;
        ...
        private readonly string[] compatibleGames = { "Initial D - Arcade Stage 6 Double Ace" };
        ...
        void Start()
        {
            ...
            id6XObject = transform.Find("id6X");
            if (id6XObject != null)
            {
                logger.Info("id6X object found.");
                id6XStartPosition = id6XObject.position;
                id6XStartRotation = id6XObject.rotation;

                // Find id6Y object under id6X
                id6YObject = id6XObject.Find("id6Y");
                if (id6YObject != null)
                {
                    logger.Info("id6Y object found.");
                    id6YStartPosition = id6YObject.position;
                    id6YStartRotation = id6YObject.rotation;

                    // Find id6Z object under id6Y
                    id6ZObject = id6YObject.Find("id6Z");
                    if (id6ZObject != null)
                    {
                        logger.Info("id6Z object found.");
                        id6ZStartPosition = id6ZObject.position;
                        id6ZStartRotation = id6ZObject.rotation;

                        // Find id6taillight1 object under id6Z
                        id6taillight1Object = id6ZObject.Find("id6taillight1");
                        if (id6taillight1Object != null)
                        {
                            logger.Info("id6taillight1 object found.");
                            // Ensure the id6taillight1 object is initially off
                            Renderer renderer = id6taillight1Object.GetComponent<Renderer>();
                            if (renderer != null)
                            {
                            ...
                            {
                                logger.Debug("Renderer component is not found on id6taillight1 object.");
                            }
                        }
                        else
                        {
                            logger.Debug("id6taillight1 object not found under id6Z.");
                        }

                        // Find id6taillight2 object under id6Z
                        id6taillight2Object = id6ZObject.Find("id6taillight2");
                        if (id6taillight2Object != null)
                        {
                            logger.Info("id6taillight2 object found.");
                            // Ensure the id6taillight2 object is initially off
                            Renderer renderer = id6taillight2Object.GetComponent<Renderer>();
                            if (renderer != null)
                            {
                                renderer.material.DisableKeyword("_EMISSION");
                            }
                            ...
                            {
                                logger.Debug("Renderer component is not found on id6taillight2 object.");
                            }
                        }
                        else
                        {
                            logger.Debug("id6taillight2 object not found under id6Z.");
                        }

                        // Find id6ControllerX under id6Z
                        id6ControllerX = id6ZObject.Find("id6ControllerX");
                        if (id6ControllerX != null)
                        {
                            logger.Info("id6ControllerX object found.");
                            // Store initial position and rotation of the stick
                            id6ControllerXStartPosition = id6ControllerX.transform.position; // these could cause the controller to mess up
                            id6ControllerXStartRotation = id6ControllerX.transform.rotation;

                            // Find id6ControllerY under id6ControllerX
                            id6ControllerY = id6ControllerX.Find("id6ControllerY");
                            if (id6ControllerY != null)
                            {
                                logger.Info("id6ControllerY object found.");
                                // Store initial position and rotation of the stick
                                id6ControllerYStartPosition = id6ControllerY.transform.position;
                                id6ControllerYStartRotation = id6ControllerY.transform.rotation;

                                // Find id6ControllerZ under id6ControllerY
                                id6ControllerZ = id6ControllerY.Find("id6ControllerZ");
                                if (id6ControllerZ != null)
                                {
                                    logger.Info("id6ControllerZ object found.");
                                    // Store initial position and rotation of the stick
                                    id6ControllerZStartPosition = id6ControllerZ.transform.position;
                                    id6ControllerZStartRotation = id6ControllerZ.transform.rotation;
                                }
                                else
                                {
                                    logger.Error("id6ControllerZ object not found under id6ControllerY!");
                                }
                            }
                            else
                            {
                                logger.Error("id6ControllerY object not found under id6ControllerX!");
                            }
                        }
                        else
                        {
                            logger.Error("id6ControllerX object not found under id6Z!");
                        }
                        // Gets all Light components in the target object and its children
                        Light[] allLights = id6ZObject.GetComponentsInChildren<Light>();

                        // Log the names of the objects containing the Light components and filter out unwanted lights
                        foreach (Light light in allLights)
                        {
                            if (light.gameObject.name == "id6brakelight1")
                            {
                                id6brakelight1 = light;
                                logger.Info("Included Light found in object: " + light.gameObject.name);
                            }
                            else if (light.gameObject.name == "id6brakelight2")
                            {
                                id6brakelight2 = light;
                                logger.Info("Included Light found in object: " + light.gameObject.name);
                            }
                        ...
                        cockpitCam = id6ZObject.Find("eyes/cockpitcam")?.gameObject;
                        if (cockpitCam != null)
                        {
                        ...
                        {
                            logger.Error("Cockpitcam object not found under id6Z!");
                        }
                    }
                    else
                    {
                        logger.Error("id6Z object not found under id6Y!");
                    }
                }
                else
                {
                    logger.Error("id6Y object not found under id6X!");
                }
            }
            else
            {
                logger.Error("id6X object not found!");
            }
            hazzardEmissiveObjects = new Renderer[3];
            //leftEmissiveObjects = new Renderer[1];
            //rightEmissiveObjects = new Renderer[1];

            hazzardEmissiveObjects[0] = id6ZObject.Find("id6hazard")?.GetComponent<Renderer>();
            hazzardEmissiveObjects[1] = id6ZObject.Find("id6hazardl")?.GetComponent<Renderer>();
            hazzardEmissiveObjects[2] = id6ZObject.Find("id6hazardr")?.GetComponent<Renderer>();

            //leftEmissiveObjects[0] = id6ZObject.Find("left1")?.GetComponent<Renderer>();
            //leftEmissiveObjects[1] = id6ZObject.Find("left2")?.GetComponent<Renderer>();
            //leftEmissiveObjects[2] = id6ZObject.Find("left3")?.GetComponent<Renderer>();

            //rightEmissiveObjects[0] = id6ZObject.Find("right1")?.GetComponent<Renderer>();
            //rightEmissiveObjects[1] = id6ZObject.Find("right2")?.GetComponent<Renderer>();
            //rightEmissiveObjects[2] = id6ZObject.Find("right3")?.GetComponent<Renderer>();
            // Set lights to start
        ...
        void StartFocusMode()
        {
            ...
            logger.Info("Compatible Rom Dectected, Starting Initial D - Arcade Stage 6 Double Ace...");
            logger.Info("Initial D 6 Motion Sim starting...Bunta Fujiwara the WRX is up to tempature!");
            logger.Info("Show these kids what real racing looks like!...");
        ...
        void EndFocusMode()
        {
            ...
            // Reset id6X to initial positions and rotations
            if (id6XObject != null)
            {
                id6XObject.position = id6XStartPosition;
                id6XObject.rotation = id6XStartRotation;
            }

            // Reset id6Y object to initial position and rotation
            if (id6YObject != null)
            {
                id6YObject.position = id6YStartPosition;
                id6YObject.rotation = id6YStartRotation;
            }
            // Reset id6Z object to initial position and rotation
            if (id6ZObject != null)
            {
                id6ZObject.position = id6ZStartPosition;
                id6ZObject.rotation = id6ZStartRotation;
            }
        ...
        void HandleInput(ref bool inputDetected, ref bool throttleDetected) // Pass by reference
        {
            ...
            // Handle X rotation for id6YObject and id6ControllerX (Down Arrow or primaryThumbstick.y > 0)
            ...
            if ((Input.GetKey(KeyCode.DownArrow) || primaryThumbstick.y > 0))
            {
                ...
                if (currentControllerRotationX > -controllerrotationLimitX)
                {
                    ...
                    id6ControllerX.Rotate(controllerRotateX, 0, 0);
                    currentControllerRotationX -= controllerRotateX;
                    inputDetected = true;
                }
            }

            // Handle X rotation for id6YObject and id6ControllerX (Up Arrow or primaryThumbstick.y < 0)
            ...
            // Handle Z rotation for id6XObject and id6ControllerZ (Down Arrow or primaryThumbstick.y < 0)
            ...
            if ((Input.GetKey(KeyCode.LeftArrow) || primaryThumbstick.x < 0))
            {
                ...
                if (currentControllerRotationZ > -controllerrotationLimitZ)
                {
                    ...
                    id6ControllerZ.Rotate(0, 0, controllerRotateZ);
                    currentControllerRotationZ -= controllerRotateZ;
                    inputDetected = true;
                }
            }

            // Handle Z rotation for id6XObject and id6ControllerZ (Up Arrow or primaryThumbstick.y > 0)
            ...
            if (primaryThumbstick.x < 0 && currentRotationY < rotationLimitY) // Note the change in condition
            {
                ...
                id6YObject.Rotate(0, rotateY, 0);  // Rotate Y in the opposite direction
        ...
        void CenterThrottle()
        {
            ...
            if (currentRotationX > 0)
            {
                ...
                id6XObject.Rotate(unrotateX, 0, 0);
        ...
        void CenterRotation()
        {
            ...
            if (currentRotationY > 0)
            {
                ...
                id6YObject.Rotate(0, unrotateY, 0);
            ...
            if (currentRotationZ > 0)
            {
                ...
                id6ZObject.Rotate(0, 0, unrotateZ);
            ...
            if (currentControllerRotationY > 0)
            {
                ...
                id6ControllerY.Rotate(0, unrotateY, 0);   // Rotating to reduce the rotation
            ...
            if (currentControllerRotationX > 0)
            {
                ...
                id6ControllerX.Rotate(unrotateX, 0, 0);   // Rotating to reduce the rotation
            ...
            if (currentControllerRotationZ > 0)
            {
                ...
                id6ControllerZ.Rotate(0, 0, unrotateZ);   // Rotating to reduce the rotation
        ...
        {
            if (id6brakelight1!= null)
            {
                id6brakelight1.intensity = isBright ? brightIntensity : dimIntensity;
                // logger.Info($"{id6brakelight1.name} light set to {(isBright ? "bright" : "dim")}.");
            }
            else
            {
                logger.Debug("id6brakelight1 light component is not found.");
            }
        }
        ...
        {
            if (id6brakelight2 != null)
            {
                id6brakelight2.intensity = isBright ? brightIntensity : dimIntensity;
                // logger.Info($"{id6brakelight2.name} light set to {(isBright ? "bright" : "dim")}.");
            }
            else
            {
                logger.Debug("id6brakelight2 light component is not found.");
            }
        }
        ...
        // Method to toggle the id6brakelight1 light
        void ToggleLight1(bool isActive)
        {
            if (id6brakelight1 != null)
            {
                id6brakelight1.enabled = isActive;
                //          logger.Info($"{id6brakelight1.name} light turned {(isActive ? "on" : "off")}.");
            }
            else
            {
                logger.Debug("id6brakelight1 light component is not found.");
        ...
        // Method to toggle the id6brakelight2 light
        void ToggleLight2(bool isActive)
        {
            if (id6brakelight2 != null)
            {
                id6brakelight2.enabled = isActive;
                //       logger.Info($"{id6brakelight2.name} light turned {(isActive ? "on" : "off")}.");
            }
            else
            {
                logger.Debug("id6brakelight2 light component is not found.");
        ...
        void ToggleBrakeEmissive(bool isActive)
        {
            if (id6taillight1Object != null)
            {
                ...
                if (renderer != null)
                {
                    ...
                    //    logger.Info($"id6taillight1Object object emission turned {(isActive ? "on" : "off")}.");
                }
                else
                {
                    logger.Debug("Renderer component is not found on id6taillight1Object object.");
                }
            }
            else
            {
                logger.Debug("id6taillight1Object object is not assigned.");
            }
            if (id6taillight2Object != null)
            {
                Renderer renderer = id6taillight2Object.GetComponent<Renderer>();
                if (renderer != null)
                {
                    ...
                    //           logger.Info($"id6taillight2Object object emission turned {(isActive ? "on" : "off")}.");
                }
                else
                {
                    logger.Debug("Renderer component is not found on id6taillight2Object object.");
                }
            }
            else
            {
                logger.Debug("id6taillight2Object object is not assigned.");
            }
        }
        ...
        void LogMissingObject(Renderer[] emissiveObjects, string arrayName)
        {
            for (int i = 0; i < emissiveObjects.Length; i++)
            {
                ...
                {
                    logger.Debug($"{arrayName} object at index {i} not found under id6Z.");
                }
            }
        }

        IEnumerator id6FrontAttractPattern()
        {
            while (true)
            {
        ...
        IEnumerator id6FrontHazzardPattern()
        {
            while (true)
            {
                ToggleEmissive(hazzardEmissiveObjects[0], true);
        ...
        public void StartAttractPattern()
        {
            ...
            frontCoroutine = StartCoroutine(id6FrontHazzardPattern());
        ...
        public void StartDangerPattern()
        {
          ...
          //  frontCoroutine = StartCoroutine(id6FrontAttractPattern());
