...
namespace WIGUx.Modules.sharrierMotionSim
{
	public class sharrierMotionSimController : MonoBehaviour
	{
		...
		private float StickYRotationDegrees = 20f; // Degrees for Stick rotation, adjust as needed
		...
		private Transform SeatemissiveObject;
		private Transform JoystickObject;
		private Transform StartObject;
		private Light lamp1_1_light;
		private Light lamp1_2_light;
		...
		void Start()
		{
			...
			if (lamp1_1_light) ToggleLight(lamp1_1_light, false);
			if (lamp1_2_light) ToggleLight(lamp1_2_light, false);
			if (StartObject) ToggleEmissive(StartObject.gameObject, false);
			if (JoystickObject) ToggleEmissive(JoystickObject.gameObject, false);
			if (SeatemissiveObject) ToggleEmissive(SeatemissiveObject.gameObject, false);
			// StartAttractPattern();
		}
		...
		void StartFocusMode()
		{
			...
			//  StopCurrentPatterns();
			if (JoystickObject) ToggleEmissive(JoystickObject.gameObject, true);
			if (cockpitCam != null)
			{
            ...
            if (cockpitCollider != null && inside)
            {
                ...
                if (playerVRSetup != null)
                {
                    ...
                    logger.Debug("Sega Space Harrier DX Motion Sim starting...");
					logger.Debug("Welcome To The Fantasy Zone...");
			...
			var currentLampStates = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase)
	{
		...
		void ProcessLamp1(int state)
		{
			...
			// Update lights
			if (lamp1_1_light) ToggleLight(lamp0_light, state == 1);
			if (lamp1_2_light) ToggleLight(lamp0_light, state == 1);
			// Update emissive material
			if (SeatemissiveObject) ToggleEmissive(SeatemissiveObject.gameObject, state == 1);
			// Update emissive material
			...
			if (XObject != null)
			{
				...
				if (YObject != null)
				{
					...
					if (ZObject != null)
					{
						...
						// Find Joystick object under Z
						JoystickObject = ZObject.Find("Joystick");
						if (JoystickObject != null)
						{
							logger.Debug("Joystick object found.");
							// Ensure the Joystick object is initially off
							Renderer renderer = JoystickObject.GetComponent<Renderer>();
							if (renderer != null)
							{
								renderer.material.DisableKeyword("_EMISSION");
							}
							...
							{
								logger.Debug("Renderer component is not found on Joystick object.");
							}
						}
						else
						{
							logger.Debug("Joysticl object not found under Z.");
						}
						// Find seatemissive object under Z
						SeatemissiveObject = ZObject.Find("Seatemissive");
						if (SeatemissiveObject != null)
						{
							logger.Debug("Seatemissive object found.");
							// Ensure the seatemissive object is initially off
							Renderer renderer = SeatemissiveObject.GetComponent<Renderer>();
							if (renderer != null)
							{
								renderer.material.DisableKeyword("_EMISSION");
							}
							...
							{
								logger.Debug("Renderer component is not found on Seatemissive object.");
							}
						}
						else
						{
							logger.Debug("Seatemissive object not found under Z.");
						}
					}
					else
					{
